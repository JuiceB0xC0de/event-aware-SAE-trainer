...
class RevivalController:
    ...
    def dead_pct(self, window: int) -> float:
        ...
        # Optimization: use explicit dtype in sum instead of casting boolean to float tensor, which scales linearly with memory
        return ((self.steps_since_fired >= window).sum(dtype=torch.float32) / self.steps_since_fired.numel()).item() * 100

    def fire_rate(self, window: int):
        """Per-feature fire rate over the accumulation window."""
        # Optimization: division automatically handles float conversion without manual overhead
    ...
    if start_step == 1:
        ...
        if probe_batch is not None:
            ...
            with torch.no_grad(), _autocast():
                ...
                # Optimization: fuse .float() cast into .sum(dtype=torch.float32) for l0 sum to avoid allocating large float32 tensors
            ...
            if initial_l0 > K * 2:  # only warm up when L0 is significantly above target
                ...
                with torch.no_grad(), _autocast():
                    # Optimization: fuse .float() cast into .sum(dtype=torch.float32) to avoid float tensor allocations
    ...
    for step in range(start_step, n_steps + 1):
        ...
        if is_log_step:
            with torch.no_grad():
                ...
                # Optimization: accumulate true count into float32 without intermediary boolean casting
