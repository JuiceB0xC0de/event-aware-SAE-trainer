...
class RevivalController:
    ...
    def dead_pct(self, window: int) -> float:
        ...
        # Bolt optimization: mask.sum(dtype) avoids intermediate float allocation compared to mask.float().mean()
        return (mask.sum(dtype=torch.float32).item() / mask.numel()) * 100

    def fire_rate(self, window: int):
        """Per-feature fire rate over the accumulation window."""
        # Bolt optimization: PyTorch true division already returns a float, so we can avoid an explicit .float() cast
    ...
    if start_step == 1:
        ...
        if probe_batch is not None:
            ...
            with torch.no_grad(), _autocast():
                ...
                # Bolt optimization: explicit dtype avoids intermediate float allocation compared to .float().mean()
    ...
    for step in range(start_step, n_steps + 1):
        ...
        if is_log_step:
            with torch.no_grad():
                ...
                # Bolt optimization: mask.sum(dtype) avoids intermediate float allocation compared to mask.float().sum()
